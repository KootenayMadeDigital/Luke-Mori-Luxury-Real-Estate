  ...
  useEffect(() => {
    ...
    const vertexSource = `
      ...
      void main() {
        ...
        float foldColumn = pow(abs(foldBand), 1.18) * sign(foldBand);
        float ridge = primaryFold * 0.32 + secondaryFold * 0.18 + tertiaryFold * 0.08 + foldColumn * 0.32 + foldBand2 * 0.12 + topPleat * 0.08;
        ...
        float billow = (primaryFold * (0.026 + bunch * 0.014) + secondaryFold * (0.014 + bunch * 0.010) + tertiaryFold * 0.012 + foldColumn * (0.052 + bunch * 0.028) + foldBand2 * 0.024 + topPleat * 0.024 + handWide * 0.020 + handPressure + wakePull + pullLag * 0.14) * (1.0 - u_open * 0.02) * weight;
        ...
        float sag = -pow(abs(a_local.x - 0.5) * 2.0, 2.0) * (0.024 + lowerWeight * 0.034) * (1.0 - u_open * 0.35);
        y += sag;
        y -= topWeight * (0.014 + abs(topPleat) * 0.018);
        float depth = edgeLift * 0.22 + bunch * abs(ridge) * 0.096 + abs(foldColumn) * 0.105 + abs(foldBand2) * 0.042 + abs(primaryFold) * 0.026 + abs(secondaryFold) * 0.018 + lowerWeight * abs(tertiaryFold) * 0.038 + topWeight * abs(topPleat) * 0.075 + hand * 0.085 + wake * pointerSpeed * 0.080 + abs(u_velocity) * 0.085 * verticalLag + u_lift * hand * 0.13;
    ...
    const fragmentSource = `
      ...
      void main() {
        ...
        float visibleWave = sin(foldX * 9.4 + v_local.y * 0.18 + v_side * 0.25);
        float visibleFold = visibleWave * 0.5 + 0.5;
        ...
        float visibleValley = smoothstep(0.08, 0.40, visibleFold) * (1.0 - smoothstep(0.52, 0.86, visibleFold));
        float visibleHighlight = smoothstep(0.60, 0.92, visibleFold) * (1.0 - smoothstep(0.97, 1.0, visibleFold));
        float creaseShadow = foldValley * 0.18 + visibleValley * 0.46;
        ...
        color = mix(color, espresso, creaseShadow * 0.34);
