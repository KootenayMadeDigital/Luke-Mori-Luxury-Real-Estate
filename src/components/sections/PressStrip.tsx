...
export function PressStrip() {
  return (
    <section className="tone-walnut tonal-section border-y border-[var(--color-line)] py-12 md:py-16">
      <Container>
            ...
            {pressLogos.slice(0, 6).map((logo, i) => (
              <Reveal key={logo.name} delay={i * 60}>
                <div
                  ...
                  title={logo.name}
                >
                  <div className="relative h-16 rounded-[calc(1.25rem-0.25rem)] bg-[rgba(10,11,13,0.42)] opacity-[0.62] grayscale transition-[opacity,filter] duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:opacity-100 group-hover:grayscale-0">
                    <Image
                      src={logo.src}
                      alt={logo.alt}
                      fill
