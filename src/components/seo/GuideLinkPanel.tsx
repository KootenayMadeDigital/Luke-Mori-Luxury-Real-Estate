        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {links.map((link, index) => (
            <Reveal key={link.href} delay={index * 70}>
              <Link href={link.href} className="luxury-card block h-full border border-[var(--color-line)] bg-[var(--color-bg)] p-6 transition-[transform,border-color] duration-500 hover:-translate-y-1 hover:border-[var(--color-bronze)] sm:p-7">
                <h3 className="m-0 font-serif text-[28px] font-light leading-[1.12] text-[var(--color-text)]">{link.title}</h3>
                <p className="m-0 mt-4 text-[14px] leading-[1.7] text-[var(--color-text-muted)]">{link.body}</p>
                <span className="mt-6 inline-flex text-[10px] font-semibold uppercase tracking-[0.22em] text-[var(--color-bronze)]">Read the guide</span>
              </Link>
            </Reveal>
          ))}
        </div>
