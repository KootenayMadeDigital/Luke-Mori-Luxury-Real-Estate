...
export const metadata = buildPageMetadata({
  title: "Kootenay Luxury Real Estate Buyer Guides",
  ...
  return (
    <PageLayout>
      <SubpageHero
        ...
        image={headerImages.westArmKootenayLake}
        crumbs={[{ label: "Home", href: "/" }, { label: "Buyer Guides" }]}
        meta={[
        ...
        <Container>
          <Reveal className="mb-14 grid grid-cols-1 gap-8 md:grid-cols-[0.85fr_1fr] md:items-end">
            ...
            <SectionLede align="right">
              Each guide is written to support better questions, cleaner showings, and more confident decisions. Rules vary by property, so every serious buyer should verify legal, tax, insurance, and technical details with qualified professionals.
            </SectionLede>
          </Reveal>
                  ...
                  <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
                    {guides.map((guide, index) => (
                      <Reveal key={guide.slug} delay={index * 60}>
                        <Link href={`/buyer-guides/${guide.slug}`} className="luxury-card group flex h-full flex-col border border-[var(--color-line)] bg-[var(--color-bg)] p-7 transition-[transform,border-color,box-shadow] duration-500 hover:-translate-y-1 hover:border-[var(--color-bronze)] sm:p-8">
      ...
      <InquiryCTA eyebrow="Buyer Advisory" title="Have a property in mind?" emphasis="Ask before the tour." body="Send the address, area, or property type you are weighing. Luke can help identify the questions worth answering before you spend a day on showings." />
    </PageLayout>
  );
}
